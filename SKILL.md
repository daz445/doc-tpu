...
Перед генерацией **обязательно** задай пользователю вопросы из секции «Обязательные данные» ниже. Не используй значения по умолчанию — спроси явно. Если пользователь не указал вариант, поле можно пропустить.

### Шаг 1: Заполнить `statics/report.json`
Впиши реальные ФИО студента и преподавателя.

### Шаг 2: Подготовить `content.json`
Основной текст отчёта (см. формат ниже). Подставь полученные от пользователя данные (номер лабораторной, дисциплину, тему) в заголовки и текст.

### Шаг 3: Сгенерировать
```
cd /Users/dmitrijzyranov/.claude/skills/docTPU && source .venv/bin/activate && \
python -m doc_tpu generate -b content.json -r statics/report.json -p output.docx
```

> Флаг `-r statics/report.json` автоматически подставляет шаблон (`template.snj`), формат (`docx`) и данные студента/преподавателя. Передавать `-t` и `-f` отдельно не нужно.

## Обязательные данные

Задай пользователю **все** эти вопросы перед генерацией. Если пользователь дал их в первом сообщении — используй, не повторяй вопросы.

### Титульная страница (cover_page)
| Поле | Где в docx | Пример |
|---|---|---|
| `title` | Жирный заголовок P15 | `ЛАБОРАТОРНАЯ РАБОТА № 8` |
| `subtitle` | Жирный подзаголовок P16 | `СТЕГАНОГРАФИЯ` |
| `discipline` | Строка «по дисциплине ...» P18 | `Информационная безопасность` |
| `variant` | Строка «Вариант N» P19 (если есть) | `6` |

### Персональные данные (report.json)
...
> 3. Вариант? (если есть)
> 4. Твоё ФИО?
> 5. ФИО и должность преподавателя?
...
- **Нет `-r` флаг** — без `statics/report.json` на титульной странице не будут ФИО студента/преподавателя
